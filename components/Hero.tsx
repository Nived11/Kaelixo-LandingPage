  ...
  useEffect(() => {
    ...
    function fitMobileDashboard() {
      if (window.innerWidth <= 760) {
        const baseWidth = 660;
        const sideGap = 20; 
        const available = Math.max(240, window.innerWidth - sideGap);
        // മൊബൈലിൽ മാത്രം Scale 0.28 ആക്കി കുറച്ചു (Dashboard സ്ക്രീനിൽ Perfect ആയി fit ആകും)
        const scale = Math.min(0.28, available / baseWidth); 
        
        dashboard!.style.setProperty("--mobile-scale", scale.toFixed(4));
        dashboard!.style.transform = `translateX(-50%) scale(${scale})`;
        visual!.style.height = `${Math.ceil(430 * scale + 10)}px`;
      } else {
        dashboard!.style.removeProperty("--mobile-scale");
    ...
    // ഡെസ്‌ക്‌ടോപ്പിൽ (Mouse pointer ഉള്ള സ്ക്രീനിൽ) മാത്രം Hover animation പ്രവർത്തിക്കും
  ...
  return (
    <section className="relative w-[min(1440px,92vw)] mx-auto z-10 flex flex-col justify-center min-h-[calc(100vh-76px)] py-4 sm:py-2 overflow-x-clip" id="home">
      <div className="grid grid-cols-1 xl:grid-cols-[46%_54%] items-center my-auto gap-6 xl:gap-0">
        ...
        {/* Right Visual Side Container: Mobile-ൽ Touch scroll തടസ്സപ്പെടാതിരിക്കാൻ `max-xl:pointer-events-none` നല്കിയിട്ടുണ്ട് */}
        <div className="relative h-[180px] sm:h-[280px] md:h-[440px] xl:h-[500px] mt-[10px] xl:mt-0 xl:perspective-[1300px] max-xl:pointer-events-none" id="visual" ref={visualRef}>
          {/* Globe Visual */}
          <div className="absolute w-[220px] h-[240px] sm:w-[300px] sm:h-[300px] xl:w-[460px] xl:h-[460px] rounded-full right-[-40px] sm:right-[-60px] xl:right-[-80px] top-[0px] xl:top-[10px] bg-[radial-gradient(circle_at_36%_32%,rgba(36,91,255,0.26),transparent_30%),repeating-radial-gradient(circle_at_center,transparent_0_31px,rgba(45,117,255,0.16)_33px_34px),repeating-linear-gradient(90deg,transparent_0_31px,rgba(255,0,82,0.08)_32px_33px)] border border-[rgba(67,113,255,0.14)] drop-shadow-[0_0_65px_rgba(38,86,255,0.16)] animate-[spin_48s_linear_infinite] after:content-[''] after:absolute after:inset-[18%] after:rounded-full after:border after:border-dashed after:border-[rgba(255,0,82,0.25)]"></div>
          
          {/* Red Glowing Ring Accent */}
          <div className="absolute right-1/2 translate-x-1/2 bottom-[10px] w-[180px] sm:w-[200px] xl:w-[340px] xl:right-[40px] xl:translate-x-0 xl:bottom-[10px] h-[40px] sm:h-[50px] rounded-[50%] border-[2px] border-[rgba(255,0,82,0.45)] shadow-[0_0_35px_var(--pink),inset_0_0_25px_rgba(36,91,255,0.35)] [transform:rotateX(75deg)] animate-[ringPulse_3.5s_ease-in-out_infinite] pointer-events-none"></div>
          ...
          {/* Dashboard Container: Mobile scale is now 0.28 for better fit */}
          <div className="absolute top-[0px] xl:top-[5px] left-1/2 xl:left-auto xl:right-[10px] -translate-x-1/2 xl:translate-x-0 w-[660px] h-[430px] xl:[transform:rotateY(-10deg)_rotateX(5deg)] preserve-3d xl:animate-[dashboardFloat_7s_ease-in-out_infinite] transition-transform duration-[0.16s] ease-out origin-top max-xl:[--mobile-scale:0.28] max-xl:[transform:translateX(-50%)_scale(var(--mobile-scale))] max-xl:origin-top-center max-xl:animate-none" id="dashboard" ref={dashboardRef}>
            <div className="w-full h-full rounded-[22px] p-[18px] bg-gradient-to-br from-[#0a122bf9] to-[#070d1ff0] border border-[#7b8eff5c] shadow-[0_30px_90px_rgba(0,0,0,0.5),inset_0_1px_rgba(255,255,255,0.08)] backdrop-blur-[18px] grid grid-cols-[120px_1fr] gap-[14px] overflow-hidden">
              <aside className="border-r border-[var(--line)] pr-[10px]">
                <div className="font-[800] mb-[20px] text-[15px]">
                  <b className="text-[var(--pink)]">K</b> Kaelixo
                </div>
